		...
		const struct i2c_device_id *id)
{
	...
	/* Set input source for Y1  */
	if(of_property_read_bool(node,"spread-spectrum-y1")) {
		regmap_update_bits(data->regmap, 0x02, BIT(7), 1);
		regmap_update_bits(data->regmap, 0x14, BIT(7), 0);
	} else
	...
	if(of_property_read_bool(node,"spread-spectrum-y1")) {
		regmap_write(data->regmap,0x03, 0x4);
		regmap_write(data->regmap,0x02, 0xfc);
	}
