...
static char m4_setup[10];
static int __init m4_config_setup(char *addrs) {

		strcpy(m4_setup,addrs);
		...
		void __iomem *reg, u8 shift)
{
	 ...
	 * M4 is enabled.
	 */
	if(strcmp(m4_setup,"on") == 0) {

		if(strcmp(name,"arm_a53_cg") == 0)
			printk(KERN_INFO "M4 clock enabled\n");
