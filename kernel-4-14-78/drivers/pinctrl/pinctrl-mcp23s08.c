...
//	dev_info(mcp->dev, "mcp23sxx_spi_write mcp->addr %x data= %x %x\n", mcp->addr, tmp[0], tmp[1]);
				...
				void *val, size_t val_size)
{
	...
	tx[0] = mcp->addr | 0x01;
	tx[1] = *((u8 *) reg);
//	dev_info(mcp->dev, "mcp23sxx_spi_read mcp->addr %x \n", mcp->addr);
...
static irqreturn_t mcp23s08_irq(int irq, void *data)
{
	...
	dev_info(mcp->chip.parent, "IRQ OK!!!\n");
...
//	mcp->chip.can_sleep = true;
	...
	for (i = 0; i < 8; i++) {
		mcp_read(mcp, i, &ret);
//		dev_info(dev, "mcp_read read %d reg data= %x\n", i, ret);
//		if (err)
//			goto out;
	}
...
//		status |= IOCON_HAEN | (IOCON_HAEN << 8);
		...
		status &= ~(IOCON_INTPOL | (IOCON_INTPOL << 8)); //test Active-low pin
...
//		dev_info(dev, "mcp_read read %d reg data= %x\n", i, ret);
	...
	__mcp23s08_set(mcp, BIT(1), 1);
	__mcp23s08_set(mcp, BIT(3), 1);
	__mcp23s08_set(mcp, BIT(5), 1);

	//0,2,4 - pull-up (1) INT
	//1,3,5 - open-drain (0) CS
	// 6,7  - pull-up (1)
	mcp_write(mcp, MCP_GPPU, 0xd5);

	//0,2,4 - in (1) INT
	//1,3,5 - out (0) CS
	mcp_write(mcp, MCP_IODIR, 0x15);

	//0,2,4 - in (1) INT
//	mcp_write(mcp, MCP_GPINTEN, 0x15);
	mcp_write(mcp, MCP_GPINTEN, 0x15);	//test cs change

	//0,2,4 - in (1) INT
	mcp_write(mcp, MCP_DEFVAL, 0x15);

	//0,2,4 - in (1) INT
	mcp_write(mcp, MCP_INTCON, 0x15);


//	dev_info(dev, "set gpio6 as out \n");

	ret = __mcp23s08_set(mcp, BIT(6), 6);
	if (ret == 0) {
		ret = mcp_set_mask(mcp, MCP_IODIR, BIT(6), false);
	}

//	dev_info(dev, "set gpio7 as in \n");
	mcp_set_bit(mcp, MCP_IODIR, 7, true);

//	dev_info(dev, "write to gpio6 1 \n");
	__mcp23s08_set(mcp, BIT(6), 1);

	mdelay(100);
	mcp_read(mcp, MCP_GPIO, &status);
//	dev_info(dev, "spi->irq %x mcp->irq %x read gpio7 %x \n", ((struct spi_device *)data)->irq, mcp->irq, status);
