...
#define SEQID_WD_ULBPR		14
#define SEQID_WD_RST		15
...
static void fsl_flexspi_init_lut(struct fsl_flexspi *flex)
{
	...
	/* Read ULBPR register */
        lut_base = SEQID_WD_ULBPR * 4;
        writel(LUT0(CMD, PAD1, SPINOR_OP_ULBPR) | LUT1(FSL_READ, PAD1, 0x1),
               base + FLEXSPI_LUT(lut_base));

	/* Read RST register */
        lut_base = SEQID_WD_RST * 4;
        writel(LUT0(CMD, PAD1, SPINOR_OP_RST) | LUT1(FSL_READ, PAD1, 0x1),
               base + FLEXSPI_LUT(lut_base));
...
static int fsl_flexspi_get_seqid(struct fsl_flexspi *flex, u8 cmd)
{
	...
	case SPINOR_OP_ULBPR:
		return SEQID_WD_ULBPR;
	case SPINOR_OP_RST:
		return SEQID_WD_RST;
	default:
