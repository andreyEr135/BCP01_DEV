...
#define PCF2123_OS_FLAG_RETRY_COUNT  3
...
static int pcf2123_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	u8 rxbuf[7];
	int ret;
     int retry = PCF2123_OS_FLAG_RETRY_COUNT;
    ...
    {            
    	...
    	if (ret < 0)
    		return ret;
    } while ((rxbuf[0] & OSC_HAS_STOPPED) && retry-- > 0) ;
...
static int pcf2123_reset(struct device *dev)
{
	...
	if (!(rxbuf[0] & CTRL1_STOP))
     ...
     dev_err(dev, "OS FLAG asserted: resetting to thursday 1970-01-01 00:00:00 UTC (0)\n");
     txbuf[0] = PCF2123_REG_SC;
     txbuf[1] = 0;               // seconds
     txbuf[2] = 0;               // minutes
     txbuf[3] = 0;               // hours
     ...
     txbuf[5] = 4;               // weekdays
     txbuf[6] = 1;               // months
     txbuf[7] = bin2bcd(70);     //years
     txbuf[8] = 0;               // alarm: minute
     txbuf[9] = 0;               // alarm: hour
     txbuf[10] = 0;              // alarm: day
     txbuf[11] = 0;              // alarm: weekday
     ret = pcf2123_write(dev, txbuf, sizeof(txbuf));
     if (ret < 0)
     {
        dev_err(dev, "Failed resetting time/date\n");
        return ret;
     }
