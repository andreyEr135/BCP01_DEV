...
struct bootstraps boot_straps_list[] = {
	{
		...
		.label = "boot straps from carrier board",
		.getStraps = NULL,
	},
	{
		.id = BOOT_STRAPS_DEF_REG,
                .label = "boot straps from default register",
		.getStraps = ectrl_read_double_element,

	},	
	{
                .id = BOOT_STRAPS_0x_REG,
                .label = "boot straps from straps config registers",
		.getStraps = ectrl_read_double_element,
...
#define getBoardID(client)    (ectrl_read_data ((client), FW_ID_REV_REG_H) & 0xFF) << 8 | (ectrl_read_data ((client), FW_ID_REV_REG_L) & 0xFF) 
...
#define setBoot0Reg(client, strap_id)   setStraps(client,BOOT0_REG,strap_id)
#define setBoot1Reg(client, strap_id)   setStraps(client,BOOT1_REG,strap_id)
#define setBoot2Reg(client, strap_id)   setStraps(client,BOOT2_REG,strap_id)
#define setBoot3Reg(client, strap_id)   setStraps(client,BOOT3_REG,strap_id)


#define setStrapsDefault(client,strap_id)		setStraps(client,BOOT_DEF_REG,strap_id)

#define	setStraps(client,strap_reg,strap_id)		ectrl_write_double_element(client,strap_reg,strap_reg + 0x1, \
							straps_definition[strap_id - 1 ].strap_l,straps_definition[strap_id - 1].strap_h)
...
enum BOOTDEV_ID getStraps(struct i2c_client *client, u8 reg) {
	...
	for (id=0;id< sizeof(straps_definition)/sizeof(struct ectrl_straps_define); id++) {
		if( ( straps_definition[id].strap_l == (straps_default & 0xff) ) && (straps_definition[id].strap_h == ((straps_default>>8) & 0xff) )  )	
			break;
	}
...
static int getBootDev (struct econtroller *ectrl, int boot_idx) {
	...
	switch (boot_idx) {
		case BOOT_IDX0:
			reg_val = getStraps(ectrl->client,BOOT0_REG);
			break;
		case BOOT_IDX1:
			reg_val = getStraps(ectrl->client,BOOT1_REG);
			break;
		case BOOT_IDX2:
			reg_val = getStraps(ectrl->client,BOOT2_REG);
			break;
		case BOOT_IDX3:
			reg_val = getStraps(ectrl->client,BOOT3_REG);
			break;
		case BOOT_STRAPS_DEV_SEL:
                        reg_val = getStraps(ectrl->client,BOOT_DEF_REG);
                        break;
		default:
			reg_val = -1;
	}
	...
	if (isAvaildableBootdev (ectrl, bootdev_id) < 0)
		...
		switch (boot_idx) {
			...
			case BOOT_IDX3:
				...
				break;
			case BOOT_STRAPS_DEV_SEL:
	                        setStrapsDefault(ectrl->client, (int)bootdev_id);
...
#define WDTgetTimeoutReg(client)   (ectrl_read_data ((client), WDT_TIMEOUT_REG) & 0x00FF)
...
#define WDTsave(client)         		ectrl_write_data ((client), CMD_CONF_SAVE, 1)
...
#define ECTRL_ENTRY_WDT_SAVE                  	 "save_param"
...
static int ectrl_proc_bootseq_show (struct seq_file *seq, void *offset) {
	...
	ectrl_proc_bootdev_is(ectrl->client, seq);
	mutex_unlock (&ectrl->fs_lock);
        return 0;
}
...
static ssize_t ectrl_proc_wdt_enable_write (struct file *file, const char __user *buf,
	...
	if (err_conv == 0) {
		if (en == 0 || en == 1) {
			...
			if(en == 0)
				ectrl_write_data ((ectrl->client), CMD_WD_DIS, 1);
			mutex_unlock (&ectrl->fs_lock);
		} else 
			return -EINVAL;
	}
...
/* -------------------------- PROC WDT SAVE TO PERMANENT FILE OPT (r/w) -------------------------- */

static int ectrl_proc_wdt_save_show (struct seq_file *seq, void *offset) {
	struct econtroller *ectrl = (struct econtroller *)seq->private;
...
static ssize_t ectrl_proc_wdt_save_write (struct file *file, const char __user *buf,
			size_t count, loff_t *pos) {
...
static int ectrl_proc_wdt_save_open_fs (struct inode *inode, struct file *file) {
        return single_open(file, ectrl_proc_wdt_save_show,  PDE_DATA(inode));
}


static const struct file_operations ectrl_proc_wdt_save_fops = {
        .owner = THIS_MODULE,
        .open = ectrl_proc_wdt_save_open_fs,
        .read = seq_read,
		.write = ectrl_proc_wdt_save_write,
        .llseek = seq_lseek,
        .release = single_release,
};
...
static int ectrl_add_proc_fs (struct econtroller *ectrl) {
	...
	entry = proc_create_data(ECTRL_ENTRY_WDT_SAVE, S_IWUGO,
				ectrl_watchdog_dir, &ectrl_proc_wdt_save_fops, ectrl);
	if (!entry) {
                goto remove_dir;
	} else {
		add_to_list (tmp, entry_list, ECTRL_ENTRY_WDT_SAVE, ectrl_watchdog_dir);
	}
...
static int ectrl_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	/*  i.MX8M firmware doesn't store board type yet
	 *  board_idx is fixed to 0 currently
	 */
	bidx = BOARD_C12;
...
//	}
	...
	ECTRL_INFO ("Detected board: %s firmware rev: 0x%x", board_name[bidx],ectrl->board_id);	
	...
	WDTsetTimeoutReg (ectrl->client, 60);
	WDTsetEvent(ectrl->client, 1);
//	WDTsetEnableReg (ectrl->client, 0);
	ectrl_write_data ((ectrl->client), CMD_WD_DIS, 1);
	ECTRL_INFO (" probe done");
	printk (KERN_INFO "ec: 0x%02x config %02x %x\n", ectrl_read_data (ectrl->client, BUILDREV_REG) >> 8, ectrl_read_data ((ectrl->client), WDT_CONFIG_REG), WDTgetTimeoutReg(ectrl->client));
