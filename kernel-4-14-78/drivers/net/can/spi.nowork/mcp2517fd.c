...
//	dev_info(&spi->dev, "mcp2517fd_sync_transfer gpio %d\n", gpio_get_value(spi->cs_gpio));
	...
	if(ret != 0)
		dev_info(&spi->dev, "spi_sync_transfer return %d\n", ret);
	return ret;
				     ...
				     int speed_hz)
{
	...
	if ((spi->master->flags & SPI_MASTER_HALF_DUPLEX) ||
		...
		xfer[1].len = rx_len;
		dev_info(&spi->dev, "SPI_MASTER_HALF_DUPLEX speed %d tx %x %x \n", speed_hz, tmp[0], tmp[1]);
...
//	dev_info(&spi->dev, "mcp2517fd_cmd_reset %d\n", speed_hz);
...
static int mcp2517fd_hw_probe(struct spi_device *spi)
{
	...
	u32 val;
	int ret;
//	dev_info(&spi->dev, "mcp2517fd_hw_probe \n");
	...
	if (ret)
		return ret;
//	dev_info(&spi->dev, "mcp2517fd_hw_probe val=%x\n", val);
...
int mcp2517fd_of_parse(struct mcp2517fd_priv *priv)
{
	...
	cs_gpio = of_get_named_gpio(np, "cs-gpios", 0);
	printk("found cs gpio %x\n", cs_gpio);
	ret = devm_gpio_request(&spi->dev, cs_gpio,
			DEVICE_NAME);
	if (ret) {
		...
		gpio_get_value(cs_gpio);
		gpio_set_value(cs_gpio, 1);

//		spi->cs_gpio = cs_gpio;
	}

	spi->irq = of_irq_get(np, 0);
	printk("found spi->irq %x\n", spi->irq);
