...
#define TER_ALPF		0x3
#define TEMP_INDUSTRIAL		0x2
#define GAP_TEMP_INDUSTRIAL	10*1000
/*
 * NXP TMU Registers
 */
...
static int read_temperature(struct udevice *dev, int *temp)
{
	...
	do {
		...
		if (drv_data & FLAGS_VER2) {
			...
			printf("read_temperature: drv_data %x temp %x valid %x\n", drv_data, val, valid);
		} else {
			val = readl(&pdata->regs->regs_v1.site[pdata->id].tritsr);
			valid = val & 0x80000000;
		}
		printf("read_temperature: retry %d\n", retry);
	} while (!valid && (retry > 0));
		...
		if (type) {
			if (strcmp(type, "critical") == 0) {
				...
				if (get_cpu_temp_grade(&minc, &maxc) & TEMP_INDUSTRIAL) {
               				pdata->critical = pdata->critical + GAP_TEMP_INDUSTRIAL; 
				}
			
			} else if (strcmp(type, "passive") == 0) {
				pdata->alert = fdtdec_get_int(gd->fdt_blob, trips_np, "temperature", 80);
				if (get_cpu_temp_grade(&minc, &maxc) & TEMP_INDUSTRIAL) {
                                	pdata->alert = pdata->alert + GAP_TEMP_INDUSTRIAL;
				}
			}
		}
	}
