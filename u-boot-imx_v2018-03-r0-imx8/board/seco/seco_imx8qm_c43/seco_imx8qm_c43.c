...
static iomux_cfg_t pad_enet0[] = {
	...
	SC_P_GPT1_COMPARE | MUX_MODE_ALT(3) | MUX_PAD_CTRL(ENET_NORMAL_PAD_CTRL),
};

#define FEC0_RST_PAD IMX_GPIO_NR(0,19)
#define FEC1_RST_PAD IMX_GPIO_NR(4,19)

static void setup_iomux_fec(void)
{
	if (0 == CONFIG_FEC_ENET_DEV) {
		imx8_iomux_setup_multiple_pads(pad_enet0, ARRAY_SIZE(pad_enet0));
		gpio_request(FEC0_RST_PAD, "fec0_rst");
		gpio_direction_output(FEC0_RST_PAD, 0);
		udelay(100);
		gpio_direction_output(FEC0_RST_PAD, 1);
	
	} else {
		imx8_iomux_setup_multiple_pads(pad_enet1, ARRAY_SIZE(pad_enet1));
//		gpio_request(FEC1_RST_PAD, "fec1_rst");
//		gpio_direction_output(FEC1_RST_PAD, 0);
//		udelay(100);
//		gpio_direction_output(FEC1_RST_PAD, 1);
	}
