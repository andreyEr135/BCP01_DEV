...
int board_init(void)
{
	 ...
	 /* Import SPL Micro information from spl struct */
        ...
        gpio_request(IMX_GPIO_NR(1, 2), "usr_led");

    	gpio_direction_output(IMX_GPIO_NR(1, 2), 0);

	board_qspi_init();
    printf("board_init: 0x%x\n",c12_get_hw_revision());
...
int board_early_init_f(void)
{
    	...
    	printf("Start GPIO settings \n");

    	gpio_request(IMX_GPIO_NR(3, 0), "cs_can4");

    	gpio_direction_output(IMX_GPIO_NR(3, 0), 1);

       	gpio_request(IMX_GPIO_NR(1, 5), "cs_can5");
