...
void imx_set_mac_to_fuse(int dev_id, unsigned char *mac)
{
	struct imx_mac_fuse *fuse;
	u32 offset;
	bool has_second_mac;
	...
	if (has_second_mac && dev_id == 1) {
		...
		writel(value, &fuse->mac_addr1);

		value = (mac[0] << 24) + (mac[1] << 16) + (mac[2] << 8) + mac[3];

		writel(value, &fuse->mac_addr2);

	} else {

		value = (mac[0] << 24) + (mac[1] << 16) + (mac[2] << 8) + mac[3];
		writel(value, &fuse->mac_addr0);
	}
