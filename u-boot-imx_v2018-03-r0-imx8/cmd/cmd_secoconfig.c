 ...
 */
#if defined(CONFIG_TARGET_SECO_IMX8MQ_C12) || defined(CONFIG_TARGET_SECO_IMX8MM_C61)
...
/*
 * |__________________________ CONFIG_TARGET_SECO_IMX8QM_C26_______________________|
 */
#if defined(CONFIG_TARGET_SECO_IMX8QM_C26)
static data_boot_dev_t kern_dev_list [] = {
        { DEV_EMMC,     "eMMC onboard",   ENV_KERNEL_SRC_MMC,    BOOT_DEV_ID_EMMC,    LOAD_ADDR_KERNEL_LOCAL_DEV,   "Image" },
...
#endif   /*  defined(CONFIG_TARGET_SECO_IMX8QM_C26)  */
...
static overlay_struct_mode_t overlay_video_mode_list [] = {
        ...
        { "LVDS1 1920x1080", "seco-imx8qm-c26-lvds1.dtbo"},
        { "LVDS2 1920x1080", "seco-imx8qm-c26-lvds2.dtbo"},
        { "DP 1920x1080", "seco-imx8qm-c26-dp.dtbo"},
        { "EDP LCDIF-1920x1080", "seco-imx8qm-seco-c26-lcdif-edp.dtbo"},
};
...
static overlay_struct_mode_t overlay_video_mode_list [] = {
        { "HDMI FULLHD", "no dtbo" },
        { "HDMI 4K", "no dtbo" },
        { "LVDS DCSS-1920x1080", "seco-imx8mm-c61-dcss-sn65dsi84.dtbo"},
};
...
#define MAX_PARTITION_ID 9
	...
	#define MAX_DEVICE 4
        ...
        #define MAX_DEVICE 4
#endif
#ifdef CONFIG_TARGET_SECO_IMX8QM_C26
        #define MAX_DEVICE 4
#endif
...
#endif
#if defined CONFIG_TARGET_SECO_IMX8MQ_C25 || defined CONFIG_TARGET_SECO_IMX8MQ_C20 || defined CONFIG_TARGET_SECO_IMX8QM_C26  || defined CONFIG_TARGET_SECO_IMX8QM_C43 || defined CONFIG_TARGET_SECO_IMX8MM_C61
		ch = *GET_OVERLAY_USE;
		if ( strcmp(&ch,"0") == 0 )
		        strcpy (&ch, "0");
		else
...
void select_at_partition_id (char *partition_id) {
	char ch;

	do {
		printf ("Choose the Filesystem partition\n");
		printf ("> ");
		ch = getc ();
		printf ("%c\n", ch);
...
int selection_video_overlay_files (overlay_struct_mode_t overlay_video_mode_list[], int num_element) {
	...
	if(selection & 0x1) { /* HDMI FULLHD selection */
		...
		env_set("hdptx_file","hdmitxfw.bin\0");
                env_set("options","drm_kms_helper.edid_firmware=edid/3840x2160.bin\0");
                selection = 1;
        } else if(selection & 0x10) { /* HDMI 4K selction */
...
int select_at_source (char *partition_id, char *file_name,
			 int *use_tftp) {
...
//	char *str;
	...
	strcpy (file_name, kern_dev_list[dev_selected].def_path );

	return dev_selected;
}

void select_filesystem_at_source (int dev_selected, char *partition_id, char *nfs_path,
			char *serverip_nfs , char *ipaddr_nfs, char *netmask_nfs, int *use_dhcp) {
	...
	switch ( filesystem_dev_list[dev_selected].dev_type ) {
		...
		case DEV_SATA:
		case DEV_USB:
			select_at_partition_id (partition_id);
			break;
		case DEV_NFS:
			select_nfs_parameters (ipaddr_nfs, serverip_nfs, nfs_path, netmask_nfs, use_dhcp);
			break;
...
static int do_at_config (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[]) {
 ...
 */
	if ( set_kernel) {
		kernel_selected_device = select_at_source (kernel_part_id,
				kernel_filename, &use_tftp);
 ...
 * |__________________________________ FDT ___________________________________|
*/
	fdt_selected_device = kernel_selected_device;
	fdt_part_id[0] = kernel_part_id[0];
	strcpy (fdt_filename, fdt_dev_list[fdt_selected_device].def_path);
 ...
 */
	if ( set_filesystem ) {
		select_filesystem_at_source (kernel_selected_device, filesystem_part_id,
				filesystem_path_nfs, filesystem_server_nfs, filesystem_ipaddr_nfs,
				filesystem_netmask_nfs, &filesystem_use_dhcp);

		switch (filesystem_dev_list[kernel_selected_device].dev_type) {
			case DEV_EMMC:
			case DEV_U_SD:
			case DEV_EXT_SD:
				SAVE_FILESYSTEM_DEVICE_ID(filesystem_dev_list[kernel_selected_device].device_id);
				SAVE_FILESYSTEM_PARTITION(filesystem_part_id);
				break;
			case DEV_SATA:
			case DEV_USB:
		...
		if ( filesystem_dev_list[kernel_selected_device].dev_type != DEV_NFS )
			SAVE_NFS_USE("0");

		sprintf (filesystem_boot_string, "setenv root_dev \'%s\'",
				filesystem_dev_list[kernel_selected_device].env_str);
...
U_BOOT_CMD(
	at_config, 3, 1, do_at_config,
	"Interactive setup for boot configuration.",
	"          - set whole environment\n"
	"at_config src   - set kernel,fdt,filesystem source device\n"
);
